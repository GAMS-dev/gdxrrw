...
2. [ONGOING] Wrapped P3 objects have C++ src with symbols like
xcreate and xfree.  xfree is use by R already, so we have to rename
this to something else like x2free.

3. [ONGOING] We don't use p3threads at all, so I gutted the
pthreads.[hc] files in the R src.  Can we selectively take it out of
p3compat.pas?

4. [ONGOING??] GDX depends indirectly on pal.  As a result, some of
the P3-generated C source is stuff we don't want to make public
(e.g. paldoorg.[hc]).

5. [ONGOING??] I hand-edited gdxcc.[ch] to get something that fits
...
6. [ALREADY DONE] Adjust p3 sources to use compiler-defined
...
7. [DO ONCE] The R package system contains hooks for library loading
and unloading.  We have to use these to call the unit initialization
and finalization sections for P3-generated units.

8. [DO ONCE] Much of the documentation and examples for GDXRRW is
...
9. [DO ONCE] What about the gams() call?  It's connected with the
