...
gdx <- system.file('doc','inputs.gdx',package='gdxrrw', mustWork=T)
rc <- gams(paste0(gms, " --INPUT=", gdx))
@
...
parameters.
<<origData>>=
outGDX <- system.file('doc','outputs.gdx',package='gdxrrw', mustWork=T)
outGDX <- 'outputs.gdx'
I <- rgdx.set(outGDX,'I')
J <- rgdx.set(outGDX,'J')
...
rc <- system2('gdxdiff', 'intest inputs', stdout=F)
if (0 == rc) print ("identical inputs") else print ("different inputs")
gams(paste(gms, '--INPUT intest.gdx --OUTPUT outtest.gdx'))
...
Things often fail to go as planned.  A GAMS run indicates this with a
nonzero return code.  For example, we could point to a non-existent
GDX input:
<<missingGDX>>=
